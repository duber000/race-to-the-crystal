...
class ProtocolHandler:
    ...
    def create_connect_message(
        player_name: str, client_type: ClientType = ClientType.HUMAN
    ) -> NetworkMessage:
    ...
    def create_reconnect_ack_message(
        ...
        return NetworkMessage(
            ...
            timestamp=time.time(),
            player_id=player_id,
            data={"game_id": game_id, "session_data": session_data},
        )

    @staticmethod
    def create_reconnect_failed_message(player_id: str, reason: str) -> NetworkMessage:
    ...
    def create_heartbeat() -> NetworkMessage:
        ...
        return NetworkMessage(type=MessageType.HEARTBEAT, timestamp=time.time())

    @staticmethod
    def create_heartbeat_ack(timestamp: float) -> NetworkMessage:
        """Create a HEARTBEAT_ACK message."""
        return NetworkMessage(type=MessageType.HEARTBEAT_ACK, timestamp=timestamp)
    ...
    @staticmethod
    def join_game_message(
        player_id: str, game_id: str, player_name: str = "Player"
    ) -> NetworkMessage:
    ...
    def list_games_message(player_id: str) -> NetworkMessage:
        ...
        return NetworkMessage(
            type=MessageType.LIST_GAMES, timestamp=time.time(), player_id=player_id
        )
    ...
    def create_state_update_message(update_data: Dict) -> NetworkMessage:
        ...
        """
        return NetworkMessage(
            type=MessageType.STATE_UPDATE, timestamp=time.time(), data=update_data
        )

    @staticmethod
    def create_turn_change_message(
        current_player_id: str, turn_number: int, phase: str
    ) -> NetworkMessage:
    ...
    def create_chat_message(
        player_id: str, player_name: str, message: str
    ) -> NetworkMessage:
    ...
    def create_invalid_action_message(
        action_type: str, reason: str, player_id: str
    ) -> NetworkMessage:
