...
def validate_action_fields(action_type: str, data: dict) -> str | None:
    """
    ...
    if action_type_lower == "move":
        if data.get("token_id") is None:
            return "token_id is required for MOVE"
        ...
        if (
            ...
            return "destination must be [x, y] coordinates"

    elif action_type_lower == "attack":
        if data.get("attacker_id") is None:
            return "attacker_id is required for ATTACK"
        defender_id = data.get("defender_id") or data.get("target_id")
        if defender_id is None:
            return "defender_id or target_id is required for ATTACK"

    elif action_type_lower == "deploy":
        if data.get("health_value") is None:
            return "health_value is required for DEPLOY"
        position = data.get("position")
        if not position or not isinstance(position, list) or len(position) != 2:
            return "position must be [x, y] coordinates"

    elif action_type_lower not in ("end_turn",):
        return f"Unknown action type: {action_type}. Allowed: MOVE, ATTACK, DEPLOY, END_TURN"
