...
For a group of related constants — especially sequential integers or string-backed labels — use `enum` instead. Enums replace Go's `const ( ... iota ... )` block and give you typed values, an autogenerated `String()` method, an `AllX()` iterator over the cases, and (for string-backed enums) a `ParseX` helper:
...
For closed sets of string values. Compiler generates `String()` (raw value), a
package-level `Parse<Name>` returning `(<Name>, bool)`, and the `All<Name>`
iterator. Parse at the boundary so typos fail fast.
