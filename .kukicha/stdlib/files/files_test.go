...
func TestUserDirs(t *testing.T) {
	home, herr := files.UserHomeDir()
	test.AssertNoError(t, herr)
	test.AssertTrue(t, home != "")
	config, cerr := files.UserConfigDir()
	test.AssertNoError(t, cerr)
	test.AssertTrue(t, config != "")
	cache, caerr := files.UserCacheDir()
	test.AssertNoError(t, caerr)
	test.AssertTrue(t, cache != "")
	data, derr := files.UserDataDir()
	test.AssertNoError(t, derr)
	test.AssertTrue(t, data != "")
	cfgApp, cfgAppErr := files.UserConfigPath("myapp", "")
	test.AssertNoError(t, cfgAppErr)
	test.AssertEqual(t, cfgApp, filepath.Join(config, "myapp"))
	cfgFile, cfgFileErr := files.UserConfigPath("myapp", "config.json")
	test.AssertNoError(t, cfgFileErr)
	test.AssertEqual(t, cfgFile, filepath.Join(config, "myapp", "config.json"))
	dataApp, dataAppErr := files.UserDataPath("myapp", "")
	test.AssertNoError(t, dataAppErr)
	test.AssertEqual(t, dataApp, filepath.Join(data, "myapp"))
	dataFile, dataFileErr := files.UserDataPath("myapp", "app.db")
	test.AssertNoError(t, dataFileErr)
	test.AssertEqual(t, dataFile, filepath.Join(data, "myapp", "app.db"))
	cacheApp, cacheAppErr := files.UserCachePath("myapp", "")
	test.AssertNoError(t, cacheAppErr)
	test.AssertEqual(t, cacheApp, filepath.Join(cache, "myapp"))
	cacheFile, cacheFileErr := files.UserCachePath("myapp", "blob.cache")
	test.AssertNoError(t, cacheFileErr)
	test.AssertEqual(t, cacheFile, filepath.Join(cache, "myapp", "blob.cache"))
}
