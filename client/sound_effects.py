...
def generate_sliding_sound(
    duration: float = 0.5, sample_rate: int = 44100
) -> array.array:
    """
...
def generate_mystery_bing_sound(
    duration: float = 0.3, sample_rate: int = 44100
) -> array.array:
    """
    ...
    for i in range(num_samples):
        ...
        value = int(
            32767
            * 0.8
            * (fundamental + harmonic1 + harmonic2 + harmonic3 + noise)
            * envelope
        )
        value = max(-32768, min(32767, value))
        samples.append(value)
...
def generate_generator_explosion_sound(
    duration: float = 1.2, sample_rate: int = 44100
) -> array.array:
    """
    ...
    for i in range(num_samples):
        ...
        distorted = math.tanh(
            mixed * 0.8
        )  # Softer clipping for more amplitude preservation
...
def generate_crystal_shatter_sound(
    duration: float = 1.5, sample_rate: int = 44100
) -> array.array:
    """
...
def save_sound_effect(
    samples: array.array, filename: str, sample_rate: int = 44100
) -> None:
