...
def test_chat_widget_import():
    ...
    try:
        import client.ui.chat_widget  # noqa: F401
...
def test_game_window_import():
    ...
    try:
        import client.game_window  # noqa: F401
...
def test_chat_widget_in_game_view():
    ...
    assert "chat_widget" in init_source, (
        "GameView should contain chat_widget initialization"
    )
    assert "is_network_game" in init_source, (
        "GameView should accept is_network_game parameter"
    )
    assert "network_client" in init_source, (
        "GameView should accept network_client parameter"
    )
...
def test_chat_widget_draw_method():
    ...
    assert "chat_widget.draw()" in draw_source, (
        "GameView should call chat_widget.draw() in on_draw"
    )
...
def test_chat_widget_input_handling():
    ...
    assert "chat_widget" in key_press_source, (
        "GameView should handle chat_widget input in on_key_press"
    )
    assert "on_key_press" in key_press_source, (
        "GameView should call chat_widget.on_key_press()"
    )
...
def test_chat_widget_update_method():
    ...
    assert "chat_widget.update" in update_source, (
        "GameView should update chat_widget in on_update"
    )
...
def test_chat_widget_on_text_method():
    ...
    assert "chat_widget" in text_source, (
        "GameView.on_text should handle chat_widget input"
    )
...
def test_chat_widget_conditional_creation():
    ...
    assert "if self.is_network_game:" in init_source, (
        "Chat widget should only be created for network games"
    )
    assert "self.chat_widget = None" in init_source, (
        "Chat widget should be None for local games"
    )
