...
def test_complete_3d_navigation():
    ...
    assert camera.aspect_ratio == 1280 / 720, (
        f"Expected aspect ratio {1280 / 720}, got {camera.aspect_ratio}"
    )
    ...
    assert camera.yaw == token_rotation, (
        f"Expected yaw {token_rotation}, got {camera.yaw}"
    )
    ...
    assert abs(camera.yaw - expected_yaw) < 0.01, (
        f"Expected yaw {expected_yaw}, got {camera.yaw}"
    )
    ...
    assert abs(camera.pitch - expected_pitch) < 0.01, (
        f"Expected pitch {expected_pitch}, got {camera.pitch}"
    )
    ...
    assert camera.pitch <= 89.0, (
        f"Pitch should be clamped to <= 89.0, got {camera.pitch}"
    )
    ...
    ray_origin, ray_direction = camera.screen_to_ray(
    ...
    assert view_matrix.shape == (4, 4), (
        f"View matrix should be 4x4, got {view_matrix.shape}"
    )
    assert projection_matrix.shape == (4, 4), (
        f"Projection matrix should be 4x4, got {projection_matrix.shape}"
    )

    # Matrices should be invertible
    view_det = np.linalg.det(view_matrix)
    proj_det = np.linalg.det(projection_matrix)
    assert abs(view_det) > 0.01, (
        f"View matrix should be invertible, determinant: {view_det}"
    )
    assert abs(proj_det) > 0.01, (
        f"Projection matrix should be invertible, determinant: {proj_det}"
    )
