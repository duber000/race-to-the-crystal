...
async def test_sse_primary_mode_broadcast_filtering():
    ...
    with patch.dict(os.environ, {"SSE_PRIMARY_MODE": "true"}):
        ...
        with patch.object(server, "_send_to_player", AsyncMock()) as mock_send:
            ...
            assert desktop_player_id in sent_player_ids, (
                "Desktop client should have received WebSocket update"
            )
            assert web_player_id not in sent_player_ids, (
                "Web client should NOT have received WebSocket update in SSE-primary mode"
            )

            # Verify Mercure publish
            # Can be either full state (first time) or delta
            assert (
                server.mercure_publisher.publish_full_state.called
                or server.mercure_publisher.publish_state_update.called
            )
...
async def test_sse_failure_fallback():
    ...
    with patch.dict(os.environ, {"SSE_PRIMARY_MODE": "true"}):
        ...
        with patch.object(server, "_send_to_player", AsyncMock()) as mock_send:
            ...
            sent_player_ids = [call.args[0] for call in mock_send.call_args_list]
            assert web_player_id in sent_player_ids, (
                "Web client should receive WebSocket fallback if SSE fails"
            )
